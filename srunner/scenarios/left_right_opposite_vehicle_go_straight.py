...
class LeftRightOppositeVehicleGoStraight(BasicScenario):
    """
    LeftRightOppositeVehicleGoStraight scenario:
    The ego vehicle is going straight at an intersection,
    while vehicles are crossing the intersection from the opposite left,and right intersection
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(LeftRightOppositeVehicleGoStraight, self).__init__("LeftRightOppositeVehicleGoStraight",
                                                                 ego_vehicles,
                                                                 config,
                                                                 world,
                                                                 debug_mode,
    ...
    def _create_behavior(self):
        ...
        plan3 = return_target_waypoint_list(
            CarlaDataProvider.get_map().get_waypoint(self.other_actors[2].get_location()), 0)
        # Generating waypoint list till next intersection
        waypoint = plan3[-1][0]
        wp_choice = waypoint.next(1.0)
        length = 0
