...
class CatchCutInCutOutWithSlowVehicle(BasicScenario):
    """
    CatchCutInCutOutWithSlowVehicle scenario:
    Ego vehicle is driving straight on the road
    behind vehicle accelerate cut in
    adn then cut out because of the slow car
    """
    timeout = 1200
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(CatchCutInCutOutWithSlowVehicle, self).__init__("CatchCutInCutOutWithSlowVehicle",
                                                              ego_vehicles,
                                                              config,
                                                              world,
                                                              debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        self.transform, _ = self._calculate_behind_transform(self._start_distance, self._reference_waypoint)
    ...
    def _create_behavior(self):
        ...
        accelerate = AccelerateToCatchUp(self.other_actors[0], self.ego_vehicles[0], throttle_value=1,
                                         delta_velocity=self._delta_velocity, trigger_distance=4, max_distance=500)
        ...
        if self.flag == 1:
            ...
            cut_in = LaneChange(
                ...
                distance_other_lane=50, avoid_collision=True)
        # cut_in = py_trees.composites.Parallel(policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        # cut_in.add_child(WaypointFollower(self.other_actors[1], self._velocity2))
        # cut_in.add_child(lane_change)
        sequence.add_child(cut_in)

        # drive until reach trigger_dis with slow vehicle
        ...
        behaviour.add_child(WaypointFollower(self.other_actors[1], self._velocity2))
        # build tree
