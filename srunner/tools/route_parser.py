...
class RouteParser(object):
    ...
    def parse_routes_file(route_filename, scenario_file, single_route=None):
        ...
        for route in tree.iter("route"):
            ...
            # jsonfile name as the scenario name
            filename = os.path.splitext(os.path.split(scenario_file)[1])
            new_config.name = filename[0]
            ...
            for scenario in scenarios:  # For each existent scenario
                ...
                for event in scenario["available_event_configurations"]:
                    ...
                    if match_position is not None:
                        ...
                        # # It is used to control the position of the actor generation, for example (in the scene where
                        # there is a vehicle on the opposite side of the intersection when turning left,
                        # the distance from other vehicles to the intersection is _start_distance)
                        if '_start_distance' in event:
                            _start_distance = event['_start_distance']
                        else:
                            _start_distance = None
                        if '_start_distance2' in event:
                            _start_distance2 = event['_start_distance2']
                        else:
                            _start_distance2 = None
                        if '_start_distance3' in event:
                            _start_distance3 = event['_start_distance3']
                        else:
                            _start_distance3 = None

                        # actor's speed
                        if '_actor_vel' in event:
                            _actor_vel = event['_actor_vel']
                        else:
                            _actor_vel = None
                        if '_actor_vel2' in event:
                            _actor_vel2 = event['_actor_vel2']
                        else:
                            _actor_vel2 = None
                        if '_actor_vel3' in event:
                            _actor_vel3 = event['_actor_vel3']
                        else:
                            _actor_vel3 = None

                        # _trigger_distance
                        if '_trigger_distance' in event:
                            _trigger_distance = event['_trigger_distance']
                        else:
                            _trigger_distance = None
                        if '_trigger_distance2' in event:
                            _trigger_distance2 = event['_trigger_distance2']
                        else:
                            _trigger_distance2 = None

                        # actor's braking coefficient [0-1]
                        if '_brake' in event:
                            _brake = event['_brake']
                        else:
                            _brake = None

                        # model ped=0 or bike=1
                        if '_model' in event:
                            _model = event['_model']
                        else:
                            _model = None

                        # control_loss noise for steer
                        if '_new_steer_noise' in event:
                            _new_steer_noise = event['_new_steer_noise']
                        else:
                            _new_steer_noise = None
                        if '_new_steer_noise2' in event:
                            _new_steer_noise2 = event['_new_steer_noise2']
                        else:
                            _new_steer_noise2 = None
                        # control_loss noise for throttle
                        if '_new_throttle_noise' in event:
                            _new_throttle_noise = event['_new_throttle_noise']
                        else:
                            _new_throttle_noise = None
                        ...
                        # _traffic_distance
                        if '_traffic_distance' in event:
                            _traffic_distance = event['_traffic_distance']
                        else:
                            _traffic_distance = None
                        ...
                        scenario_description = {
                            ...
                            '_start_distance': _start_distance,
                            '_start_distance2': _start_distance2,
                            '_start_distance3': _start_distance3,
                            # actor's speed
                            '_actor_vel': _actor_vel,
                            '_actor_vel2': _actor_vel2,
                            '_actor_vel3': _actor_vel3,
                            # _trigger_distance for scenario start
                            '_trigger_distance': _trigger_distance,
                            '_trigger_distance2': _trigger_distance2,

                            '_brake': _brake,
                            '_model': _model,
                            '_new_steer_noise': _new_steer_noise,
                            '_new_steer_noise2': _new_steer_noise2,
                            '_new_throttle_noise': _new_throttle_noise,
                            'weather': weather,
                            '_traffic_distance': _traffic_distance,
