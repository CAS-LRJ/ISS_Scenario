...
from srunner.scenariomanager.scenarioatomics.atomic_behaviors import (ActorTransformSetter,
                                                                      ...
                                                                      Idle, KeepVelocity, AddNoiseToVehicle)
...
class FellowFrontVehicleWithOppositeVehicleContLoss(BasicScenario):
    """
    FellowFrontVehicleWithOppositeVehicleContLoss scenario:
    The ego vehicle is fellow a vehicle ,there is another vehicle control loss form opposite
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        if config._new_steer_noise is not None:
            ...
            self._new_steer_noise = -0.2

        if config._new_throttle_noise is not None:
            self._new_throttle_noise = config._new_throttle_noise
        else:
            self._new_throttle_noise = 0.8

        # Timeout of scenario in seconds
        self.timeout = timeout

        super(FellowFrontVehicleWithOppositeVehicleContLoss, self).__init__(
            "FellowFrontVehicleWithOppositeVehicleContLoss",
            ego_vehicles,
            config,
            world,
            debug_mode,
    ...
    def _calculate_control_loss_vehicle_transform(self, _start_distance, waypoint):
        """
        Calculate the transform of the actor
        :param (float) _start_distance: Initial position of the actor
        :param (carla.waypoint) waypoint: Position of the reference object
        ...
        while True:
            if waypoint.get_right_lane() is None or waypoint.get_right_lane().lane_type == carla.LaneType.Sidewalk:
                waypoint = waypoint
                break
            elif waypoint.get_right_lane().lane_type == carla.LaneType.Shoulder or \
                    waypoint.get_right_lane().lane_type == carla.LaneType.Parking:
                waypoint = waypoint
                break
            else:
                waypoint = waypoint.get_right_lane()
    ...
    def _initialize_actors(self, config):
        ...
        # ped
        waypoint = self._reference_waypoint
        self._other_actor_transform2, orientation_yaw = self._calculate_control_loss_vehicle_transform(
    ...
    def _create_behavior(self):
        ...
        # control_loss vehicle

        vehicle2 = py_trees.composites.Sequence()
        trigger = InTriggerDistanceToVehicle(self.other_actors[1], self.ego_vehicles[0], self.trigger_distance)
        control_loss = py_trees.composites.Parallel(
            "DrivingStraight", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL)
        control_loss.add_child(Idle(5))
        control_loss.add_child(AddNoiseToVehicle(self.other_actors[1], self._new_steer_noise, self._new_throttle_noise))
        vehicle2.add_child(trigger)
        vehicle2.add_child(control_loss)
        vehicle2.add_child(StopVehicle(self.other_actors[1], 1.0))

        ##########################################################################################################

        # Drive according to the plan
        continue_driving.add_child(WaypointFollower(
            self.other_actors[0], self.velocity, avoid_collision=True))
        continue_driving.add_child(WaypointFollower(
            self.other_actors[1], self.velocity2, avoid_collision=True))
        continue_driving.add_child(vehicle2)
