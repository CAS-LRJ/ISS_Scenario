...
class FrontVehicleTurnRWithBikeCrossIntersectionObliquely(BasicScenario):
    """
    FrontVehicleTurnRWithBikeCrossIntersectionObliquely scenario:
    The ego vehicle is driving straight at an intersection,
    while front vehicle turn right and a cyclist cross intersection obliquely
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        # Initial position of the front vehicle
        ...
        super(FrontVehicleTurnRWithBikeCrossIntersectionObliquely, self).__init__(
    ...
    def _calculate_left_pedestrian_transform(self, _start_distance, waypoint):
        ...
        offset = {"orientation": -90, "position": 90, "z": 0.2, "k": 1.0}
        position_yaw = waypoint.transform.rotation.yaw + offset['position']
        orientation_yaw = waypoint.transform.rotation.yaw + offset['orientation']
        offset_location = carla.Location(
            offset['k'] * lane_width * math.cos(math.radians(position_yaw)),
    ...
    def _initialize_actors(self, config):
        ...
        self._other_actor_transform, orientation_yaw = self._calculate_left_pedestrian_transform(self._start_distance,
                                                                                                 waypoint)
        ...
        # front vehicle
        waypoint = self._reference_waypoint
        self._other_actor_transform2, orientation_yaw = self._calculate_base_transform(self._start_distance2, waypoint)
        ...
        veh = CarlaDataProvider.request_new_actor('vehicle.carlamotors.carlacola', veh_transform)
        veh.set_simulate_physics(True)
        self.other_actors.append(veh)
    ...
    def _create_behavior(self):
        ...
        continue_driving_waypoints1 = KeepVelocity(self.other_actors[0], self.velocity, duration=1.5,
                                                   _avoid_collision=True, steer=self._new_steer_noise)
        continue_driving_waypoints2 = KeepVelocity(self.other_actors[0], self.velocity, duration=15,
        ...
        vehicle_drive = py_trees.composites.Sequence()
        # Selecting right path at intersection     # vehicle's plan
        ...
        while not wp_choice[0].is_intersection:
            ...
            wp_choice = waypoint.next(1.0)
            length = length + 1
        vehicle_drive.add_child(WaypointFollower(
            self.other_actors[1], self.velocity2, plan=plan, avoid_collision=True))

        # Drive according to the plan
        continue_driving.add_child(ped_cross)
        continue_driving.add_child(vehicle_drive)
