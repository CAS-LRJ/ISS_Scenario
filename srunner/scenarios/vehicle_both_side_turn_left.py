...
class VehicleBothSideTurnLeft(BasicScenario):
    """
    VehicleBothSideTurnLeft scenario:
    The ego vehicle is going straight at an intersection,
    while vehicles are turing left from the left and right intersection
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(VehicleBothSideTurnLeft, self).__init__("VehicleBothSideTurnLeft",
                                                      ego_vehicles,
                                                      config,
                                                      world,
                                                      debug_mode,
    ...
    def _create_behavior(self):
        ...
        #########################################################################################################
        right_vehicle_turn_left = py_trees.composites.Parallel(
        ...
        right_vehicle_turn_left.add_child(TimeOut(500))

        ##########################################################################################################
        left_vehicle_turn_left = py_trees.composites.Parallel(
            "left_vehicle_turn_left",
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        # adding flow of actors
        actor_source = ActorSource(
            ['vehicle.tesla.model3', 'vehicle.audi.tt'],
            self._other_actor_transform2, 5, self._blackboard_queue_name, 2)
        # destroying flow of actors
        actor_sink = ActorSink(plan2[-3][0].transform.location, 2)
        # follow waypoints until next intersection
        continue_driving_waypoints2 = WaypointFollower(
            self.other_actors[1], self.velocity2, plan=plan2, blackboard_queue_name=self._blackboard_queue_name,
            avoid_collision=True)
        left_vehicle_turn_left.add_child(actor_source)
        left_vehicle_turn_left.add_child(actor_sink)
        left_vehicle_turn_left.add_child(continue_driving_waypoints2)
        # Remove the right vehicle when it has completed the planned route
        left_vehicle_turn_left.add_child(ActorSink(plan[-1][0].transform.location, 2))

        ##########################################################################################################
        # Drive according to the plan
        continue_driving.add_child(right_vehicle_turn_left)
        continue_driving.add_child(left_vehicle_turn_left)

        # remove the last vehicle of traffic
        remove_last_vehicle = py_trees.composites.Parallel(
            "remove_last_vehicle",
            policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        remove_last_vehicle.add_child(ActorSink(plan2[-3][0].transform.location, 2))
        remove_last_vehicle.add_child(TimeOut(1))
        ...
        sequence.add_child(remove_last_vehicle)
        sequence.add_child(wait)
        return sequence
