...
class VehicleCrossBothSideWithPedCross(BasicScenario):
    """
    VehicleCrossBothSideWithPedCross scenario:
    The ego vehicle is going straight at an intersection,
    while vehicles are crossing the intersection from the left and right intersection
    and a pedestrian is crossing the road at opposite intersection
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(VehicleCrossBothSideWithPedCross, self).__init__("VehicleCrossBothSideWithPedCross",
                                                               ego_vehicles,
                                                               config,
                                                               world,
                                                               debug_mode,
    ...
    def _calculate_opposite_transform(self, _start_distance, waypoint):
        ...
        # print(waypoint.get_left_lane().get_right_lane().lane_type)
        # print(waypoint.get_left_lane().get_left_lane().lane_type)
        # print(waypoint.get_left_lane().get_left_lane().get_right_lane().lane_type)
        # print(waypoint.get_left_lane().get_left_lane().lane_id)
        # self.debug.draw_point(waypoint.get_left_lane().get_left_lane().get_left_lane().transform.location + carla.Location(z=0.5), size=0.5, life_time=0)
    ...
    def _initialize_actors(self, config):
        ...
        # pedestrian
        waypoint = self._reference_waypoint
        self._other_actor_transform3, orientation_yaw = self._calculate_opposite_transform(self._start_distance3,
                                                                                           waypoint)
    ...
    def _create_behavior(self):
        ...
        pedestrian.add_child(KeepVelocity(self.other_actors[2], self.velocity3, distance=20, _avoid_collision=True))
