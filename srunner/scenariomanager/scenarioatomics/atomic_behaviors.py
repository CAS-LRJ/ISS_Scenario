...
class KeepVelocity(AtomicBehavior):
    ...
    def __init__(self, actor, target_velocity, duration=float("inf"), distance=float("inf"), reverse=False
                 , name="KeepVelocity", _avoid_collision=False, steer=0):
        """
        Setup parameters including acceleration value (via throttle_value)
        and target velocity
        """
        ...
        self._avoid_collision = _avoid_collision
        self.steer = steer
    ...
    def update(self):
        ...
        if self._type == 'vehicle':
            if CarlaDataProvider.get_velocity(self._actor) < self._target_velocity:
                ...
                if self._avoid_collision and detect_lane_obstacle(self._actor):
...
class SetTrafficLightGreen(AtomicBehavior):
    """
    set the vehicle light front the actor to red
    Important parameters:
    - actor:
    - actor1:
    """

    def __init__(self, actor, actor1=None, name="SetTrafficLightGreen"):
        """
        Setup _actor
        """
        super(SetTrafficLightGreen, self).__init__(name, actor)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._control, self._type = get_actor_control(actor)
        self._traffic_light = None
        self._traffic_light1 = None
        self.actor1 = actor1
    ...
    def update(self):
        ...
        set green light
        """
        self._traffic_light = CarlaDataProvider.get_next_traffic_light(self._actor, False)
        if self._traffic_light is not None:
            self._traffic_light.set_state(carla.TrafficLightState.Green)
            self._traffic_light.set_green_time(30)

        if self.actor1 is not None:
            self._traffic_light1 = CarlaDataProvider.get_next_traffic_light(self.actor1, False)
            if self._traffic_light1 is not None:
                self._traffic_light1.set_state(carla.TrafficLightState.Green)
                self._traffic_light1.set_green_time(30)
...
class SetTrafficLightRed(AtomicBehavior):
    """
    set the vehicle light front the actor to red
    Important parameters:
    - actor: actor
    ...
    def __init__(self, actor, name="SetTrafficLightRed"):
        """
        Setup _actor
        """
        super(SetTrafficLightRed, self).__init__(name, actor)
        self.logger.debug("%s.__init__()" % (self.__class__.__name__))
        self._control, self._type = get_actor_control(actor)
        self._traffic_light = None
    ...
    def update(self):
        ...
        if self._traffic_light is not None:
            ...
            self._traffic_light.set_red_time(30)
        new_status = py_trees.common.Status.SUCCESS
        self.logger.debug("%s.update()[%s->%s]" % (self.__class__.__name__, self.status, new_status))
        return new_status
...
class WaypointFollower(AtomicBehavior):
    ...
    def __init__(self, actor, target_speed=None, plan=None, blackboard_queue_name=None,
                 avoid_collision=False, reverse=False, name="FollowWaypoints"):
        """
        ...
        for actor in self._local_planner_dict:
            ...
            if actor is not None and actor.is_alive and local_planner is not None:
                ...
                if not isinstance(actor, carla.Walker):
                    ...
                    # 如果给的plan还没有走完，就继续循环，如果走完就success结束
...
class LaneChange(WaypointFollower):
    ...
    def __init__(self, actor, speed=10, direction='left',
        ...
        super(LaneChange, self).__init__(actor, target_speed=speed, name=name, avoid_collision=avoid_collision)
