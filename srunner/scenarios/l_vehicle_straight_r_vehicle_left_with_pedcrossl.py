...
class LVehicleStraightRightVehicleTLWithPedCrossL(BasicScenario):
    """
    LVehicleStraightRightVehicleTLWithPedCrossL scenario:
    The ego vehicle is turning left at an intersection,
    while a vehicle is turning left from the right and another vehicle go straight from the left intersection
    and a pedestrian is crossing the road at left intersection
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(LVehicleStraightRightVehicleTLWithPedCrossL, self).__init__(
    ...
    def _calculate_left_transform(self, _start_distance, waypoint):
        ...
        # first move to the left lane
        flag = waypoint.lane_id
        if waypoint.get_left_lane() is None or waypoint.get_left_lane().lane_type == carla.LaneType.Sidewalk or \
                waypoint.get_left_lane().lane_type == carla.LaneType.Bidirectional:
            waypoint = waypoint
        elif waypoint.get_left_lane().lane_type == carla.LaneType.Shoulder or \
                waypoint.get_left_lane().lane_type == carla.LaneType.Parking:
            waypoint = waypoint
        elif waypoint.get_left_lane() is not None and waypoint.get_left_lane().lane_id * flag > 0:
            waypoint = waypoint.get_left_lane()
        # Get the waypoint of the left intersection
    ...
    def _initialize_actors(self, config):
        ...
        # opposite vehicle
        waypoint = self._reference_waypoint
        self._other_actor_transform2, orientation_yaw = self._calculate_left_transform(self._start_distance2,
                                                                                       waypoint)
        ...
        # pedestrian
        waypoint = self._reference_waypoint
        self._other_actor_transform3, orientation_yaw = self._calculate_pedestrian_transform(self._start_distance3,
                                                                                             waypoint)
        ...
        pedestrian = CarlaDataProvider.request_new_actor('walker.pedestrian.0001', pedestrian_transform)
    ...
    def _create_behavior(self):
        ...
        #########################################################################################################
        right_vehicle_turn_left = py_trees.composites.Sequence()
        continue_driving_waypoints = WaypointFollower(
            self.other_actors[0], self.velocity, plan=plan, avoid_collision=True)
        right_vehicle_turn_left.add_child(continue_driving_waypoints)
        right_vehicle_turn_left.add_child(StopVehicle(self.other_actors[0], 1.0))

        ##########################################################################################################
        left_vehicle_go_straight = py_trees.composites.Sequence()
        ...
        left_vehicle_go_straight.add_child(continue_driving_waypoints2)
        left_vehicle_go_straight.add_child(StopVehicle(self.other_actors[1], 1.0))

        ##########################################################################################################
        pedestrian = py_trees.composites.Sequence()
        pedestrian.add_child(
            InTriggerDistanceToVehicle(self.ego_vehicles[0], self.other_actors[2], self.trigger_distance2))
        pedestrian.add_child(KeepVelocity(self.other_actors[2], self.velocity3, duration=10, _avoid_collision=True))

        ##########################################################################################################
        # Drive according to the plan
        continue_driving.add_child(right_vehicle_turn_left)
        continue_driving.add_child(left_vehicle_go_straight)
        continue_driving.add_child(pedestrian)
        ...
        # left vehicle
        sequence.add_child(ActorTransformSetter(self.other_actors[2], self._other_actor_transform3, physics=True))
