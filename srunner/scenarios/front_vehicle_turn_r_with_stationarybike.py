...
class FrontVehicleTurnRWithStationBike(BasicScenario):
    """
    FrontVehicleTurnRWithStationBike scenario:
    The ego vehicle is going straight at an intersection,
    while after the front vehicle turn right, there is a stationary bicycle
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        super(FrontVehicleTurnRWithStationBike, self).__init__("FrontVehicleTurnRWithStationBike",
                                                               ego_vehicles,
                                                               config,
                                                               world,
                                                               debug_mode,
    ...
    def _calculate_bicycle_transform(self, _start_distance, waypoint):
        """
        Calculate the transform of the actor
        :param (float) _start_distance: Initial position of the actor
        :param (carla.waypoint) waypoint: Position of the reference object
        ...
        #         waypoint = waypoint.get_right_lane()
        location = waypoint.transform.location
        offset = {"orientation": -90, "position": 25, "z": 0, "k": 1.0}
        position_yaw = waypoint.transform.rotation.yaw + offset['position']
        orientation_yaw = waypoint.transform.rotation.yaw + offset['orientation']
        offset_location = carla.Location(
            offset['k'] * lane_width * math.cos(math.radians(position_yaw)),
    ...
    def _initialize_actors(self, config):
        ...
        waypoint = self._wmap.get_waypoint(location)
        self._other_actor_transform2, orientation_yaw = self._calculate_bicycle_transform(self._start_distance2,
                                                                                          waypoint)
