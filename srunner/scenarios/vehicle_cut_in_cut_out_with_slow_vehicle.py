...
class VehicleCutInCutOutWithSlowVehicle(BasicScenario):
    """
    VehicleCutInCutOutWithSlowVehicle scenario:
    Ego vehicle is driving straight on the road
    beside vehicle cut in because of the front slow car,drive for a while
    cutout because of another front slow car
    """
    timeout = 1200
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        if config._actor_vel3 is not None:
            self._velocity3 = config._actor_vel3
        else:
            self._velocity3 = 9
        ...
        if config._trigger_distance2 is not None:
            self._trigger_distance2 = config._trigger_distance2
        else:
            self._trigger_distance2 = 15
        ...
        super(VehicleCutInCutOutWithSlowVehicle, self).__init__("VehicleCutInCutOutWithSlowVehicle",
                                                                ego_vehicles,
                                                                config,
                                                                world,
                                                                debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        # ego same lane vehicle
        self.transform_obstacle2, _ = self._calculate_base_transform(self._start_distance3, self._reference_waypoint)
        obstacle_transform2 = carla.Transform(
            ...
            self.transform_obstacle2.rotation)
        obstacle2 = CarlaDataProvider.request_new_actor('vehicle.tesla.model3', obstacle_transform2)
    ...
    def _create_behavior(self):
        ...
        car3_visible = ActorTransformSetter(self.other_actors[2], self.transform_obstacle2)
        ...
        if self.flag == 1:
            cut_in = LaneChange(
                ...
                distance_other_lane=10, avoid_collision=True)
        else:
            cut_in = LaneChange(
        ...
        # cut out trigger
        just_drive2 = py_trees.composites.Parallel(
            "DrivingStraight", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        just_drive2.add_child(
            InTriggerDistanceToVehicle(self.other_actors[1], self.other_actors[2], self._trigger_distance2))
        just_drive2.add_child(WaypointFollower(self.other_actors[1], self._velocity2, avoid_collision=True))
        sequence.add_child(just_drive2)
        # cut_out, If flag=1, cut out to the left, cut out to the right
        if self.flag == 1:
            cut_out = LaneChange(
                self.other_actors[1], speed=self._velocity2, direction='left', distance_same_lane=10,
        ...
        root.add_child(StopVehicle(self.other_actors[2], self.brake))
