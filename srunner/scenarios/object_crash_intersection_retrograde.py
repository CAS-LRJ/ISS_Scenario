...
def get_opponent_transform(added_dist, waypoint, trigger_location):
    ...
    offset = {"orientation": 180, "position": 0, "k": 1.0}
    ...
    # CarlaDataProvider.get_world().debug.draw_point(_wp.transform.location + carla.Location(z=0.5), size=0.5, life_time=0)
...
class VehicleTurningRouteWithRetrograde(BasicScenario):
    """
    VehicleTurningRouteWithRetrograde scenario:
    route version
    ego is turning at an intersection, and
    encounter a cyclist is riding against traffic
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        if config._trigger_distance is not None:
            self.bycicle_start_dist = config._trigger_distance
        else:
            self.bycicle_start_dist = 25

        super(VehicleTurningRouteWithRetrograde, self).__init__("VehicleTurningRouteWithRetrograde",
                                                                ego_vehicles,
                                                                config,
                                                                world,
                                                                debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        # start_distance = 20
        waypoint = waypoint.next(self._start_distance)[0]
    ...
    def _create_behavior(self):
        ...
        the cyclist starts riding against traffic once the condition meets,
        ego vehicle has to avoid the crash after a turn
         Order of sequence:
        - ActorTransformSetter: spawn car at a visible transform
        - trigger_distance: cyclist wait for ego enter the trigger distance region
        - actor_ego_sync: cyclist reportedly ride the wrong way
        - end_condition:  cyclist stop and wait
        ...
        if self._ego_route is not None:
            waypoint = self._wmap.get_waypoint(self._other_actor_transform.location)
            # self.debug.draw_point(waypoint.transform.location + carla.Location(z=0.5), size=0.5, life_time=0)
            trigger_distance = InTriggerDistanceToLocationAlongRoute(self.ego_vehicles[0],
                                                                     self._ego_route,
                                                                     self._other_actor_transform.location,
                                                                     self.bycicle_start_dist)
        else:
            trigger_distance = InTriggerDistanceToVehicle(self.other_actors[0],
                                                          self.ego_vehicles[0],
                                                          self.bycicle_start_dist)

        actor_velocity = KeepVelocity(self.other_actors[0], self._other_actor_target_velocity, _avoid_collision=True)
        actor_traverse = DriveDistance(self.other_actors[0], 10)
        post_timer_velocity_actor = KeepVelocity(self.other_actors[0], self._other_actor_target_velocity,
                                                 _avoid_collision=True)
        post_timer_traverse_actor = DriveDistance(self.other_actors[0], 10)
